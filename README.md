...
このリポジトリの Release から `MirrorPhone-Setup-v0.2.3-update-install.exe`
...
```text
MirrorPhone-Setup-v0.2.3-update-install.exe
```
...
- 既存更新時は `node_modules` と `vendor` を保持するため、ElectronやAirPlayエンジンの再取得を避けられる場合があります。
